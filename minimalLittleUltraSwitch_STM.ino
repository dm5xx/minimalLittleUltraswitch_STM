...
int pinsDef[16] = { PB8, PB7, PB6, PB5, PB4, PB3, PA15, PA12, PA0, PA1, PA2, PA3, PA4, PA5, PA6, PA7};
...
void updateRelays(RelayCard &bank, int bankNr)
{
  for (int i = 0; i < 16; i++)
  {
    if(bank.pinStatus[i] == 1)
       digitalWrite(pinsDef[bank.pins[i]], LOW);
    else
       digitalWrite(pinsDef[bank.pins[i]], HIGH);
  } 
}
...
{
   if(value == 1)
    digitalWrite(pinsDef[bank.pins[pin]], LOW);
   else
    digitalWrite(pinsDef[bank.pins[pin]], HIGH);
}
...
void setup()
{
  ...
  for(int a = 0; a < NUMBEROFBOARDS; a++)
  {
    ...
    for (int p = 0; p < 16; p++)
    {
      ...
      //delay(20);
        pinMode(pinsDef[p], OUTPUT);
        digitalWrite(pinsDef[p], HIGH);        
    }
  }    
